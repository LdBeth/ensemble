...
module Make(Member: OrderedMember) (Key : OrderedKey) : (S with type member = Member.t and type key=Key.t) = struct
  ...
  let merge l = 
    ...
    let less = fun x y -> compare (h x) (h y) in
    ...
    let rec mrg al acc l = match l with 
	  ...
	  match List.merge less [acc] tl with (*PERF*)
  ...
  let le keymap k1 k2 = 
    (*try *)
      compare (h (Hashtbl.find keymap k1)) (h (Hashtbl.find keymap k2))
  ...
  let mcast1_opt h key_table orig_map = 
    Hashtbl.iter (fun member memrec -> 
      let cast1 = List.sort (fun (kl1,key1) (kl2,key2) -> le key_table key1 key2)
		    memrec.cast1 in
  ...
  *)
  let key_paths t =
    ...
    let rec loop keyl t = match t with 
      | Nil -> ()
      | Leaf member -> Hashtbl.add kp member (List.sort compare keyl)
  ...
  let self_check verbose t (mm_tbl,orig_map,key_table,_) = 
	...
	let know = remove_dup (List.sort compare know) in
