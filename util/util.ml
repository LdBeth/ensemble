...
let hex_of_string s =
  ...
  let h = Bytes.create (2 * n) in
  for i = 0 to pred n do
    let c = s.[i] in
    let c = Char.code c in
    let c = sprintf "%02X" c in
    Bytes.blit_string c 0 h (2 * i) 2
  done ;
  Bytes.to_string h
...
let array_flatten a =
  ...
  if len = 0 then [||] else (
    assert (!init >= 0) ;
    let b = Array.make len (a.(!init).(0)) in
    let j = ref 0 in
    for i = 0 to pred n do
      let l = Array.length a.(i) in
...
let string_of_id debug mapping key =
  ...
  try
    ...
    String.capitalize_ascii (String.lowercase_ascii data)
  with Not_found ->
    failwith (sprintf "%s:string_of_id:bad id" debug)

let id_of_string debug mapping key =
  let mapping = Array.to_list mapping in
  let tmp = String.uppercase_ascii key in
  try
