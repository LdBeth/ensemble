...
let udp_send sendto_info buf ofs len =
  Socket.udp_send sendto_info (Buf.bytes_of buf) (int_of_len ofs) (int_of_len len)
    
let udp_mu_sendsv info buf ofs len iovl = 
  Socket.udp_mu_sendsv info 
    (Buf.bytes_of buf) (Buf.int_of_len ofs) (Buf.int_of_len len)
    (Iovecl.to_iovec_array iovl) 
    
(* PERF: I -hope- ocamlopt removes the allocation.
*)
let udp_mu_recv_packet pool sock mlhdr= 
(*  Socket.udp_mu_recv_packet sock (Buf.bytes_of mlhdr)*)
  let mllen, iov = Socket.udp_mu_recv_packet pool sock (Buf.bytes_of mlhdr) in
  (Buf.len_of_int mllen), iov
    
let tcp_recv s b o l 	= 
  len_of_int (Socket.tcp_recv s (Buf.bytes_of b) (int_of_len o) (int_of_len l))
...
let tcp_recv_packet sock buf ofs len iov = 
  len_of_int (Socket.tcp_recv_packet sock 
    (Buf.bytes_of buf) (int_of_len ofs) (int_of_len len) iov)
    
let tcp_send s b o l = 
  len_of_int (Socket.tcp_send s (Buf.bytes_of b) (int_of_len o) (int_of_len l))
    
let tcp_sendv s iovl = 
  len_of_int (Socket.tcp_sendv s (Iovecl.to_iovec_array iovl))
    
let tcp_sendsv s buf ofs len iovl = 
  len_of_int (Socket.tcp_sendsv s (Buf.bytes_of buf)
...
let tcp_sends2v s buf1 buf2 ofs len iovl = 
  len_of_int (Socket.tcp_sends2v s
    (Buf.bytes_of buf1)
    (Buf.bytes_of buf2) (Buf.int_of_len ofs) (Buf.int_of_len len)
    (Iovecl.to_iovec_array iovl))
...
let substring_eq s1 ofs1 s2 ofs2 len = 
  Socket.substring_eq 
    (Buf.bytes_of s1) (int_of_len ofs1)
    (Buf.bytes_of s2) (int_of_len ofs2) (int_of_len len)
...
let md5_update ctx buf ofs len = 
  Socket.md5_update ctx (Buf.bytes_of buf) (Buf.int_of_len ofs) (Buf.int_of_len len)
...
let recvfrom s b o l = 
  let l,sa = Socket.recvfrom s (Buf.bytes_of b) (Buf.int_of_len o) (Buf.int_of_len l) in
  match sa with 
...
let string_of_inet inet =
  try
    let {Unix.h_name=long_name; _} =
      Unix.gethostbyaddr inet 
    in
    let name = String.lowercase_ascii long_name in
    name
  with _ ->
...
let read_ch ch =
  ...
  begin 
    try while true do
      ...
      let len = input ch buf 0 (Bytes.length buf) in
      if len = 0 then 
      	raise End_of_file ;
      l := (Bytes.sub_string buf 0 len) :: !l
