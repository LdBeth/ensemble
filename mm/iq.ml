...
let grow iq nlen =
  ...
  let nlen = 
    let len = max nlen olen in
    let pow = log2 (2*len-1) in
    let len = pow2 pow in		(* always a power of 2 *)
    let len = max 16 len in		(* always grow to at least 16 *)
    len
  in
  ...
  let na = Array.make nlen init in
...
let [@inline never] overflow iq new_hi =
