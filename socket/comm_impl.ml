...
external substring_eq : bytes -> ofs -> bytes -> ofs -> len -> bool
...
external udp_mu_recv_packet_into_buf : socket -> bytes -> int
  = "skt_udp_mu_recv_packet_into_buf" [@@noalloc]
...
external udp_mu_recv_packet : socket -> ret_len -> bytes -> Ciovec.cbuf -> int -> unit
...
(*    print_string 
    ...
    flush stdout;*)
    udp_mu_recv_packet_into_buf sock ml_prealloc_buf, Ciovec.empty
  )    
...
external tcp_recv : socket -> bytes -> ofs -> len -> len
  = "skt_tcp_recv" [@@noalloc]

external tcp_recv_iov : socket -> Ciovec.t -> ofs -> len -> len
  = "skt_tcp_recv_iov" [@@noalloc]

external tcp_recv_packet : socket -> bytes -> ofs -> len -> Ciovec.t -> len
