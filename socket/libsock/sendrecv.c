 ...
 */
value skt_udp_mu_recv_packet_into_buf(value sock_v, value buf_v)
{
    ocaml_skt_t sock = Socket_val(sock_v);
    int usr_len=0, ml_len=0, len=0;
    unsigned char *buf = Bytes_val(buf_v);
    ...
    ml_len = ntohl (*(uint32_t*) &(buf[0]));
    usr_len = ntohl (*(uint32_t*) &(buf[SIZE_INT32]));
...
/*    {
    ...
    if (ml_len > 0) 
        memcpy(Bytes_val(ml_buf_v), (char*)buf + HEADER_PEEK, ml_len);
